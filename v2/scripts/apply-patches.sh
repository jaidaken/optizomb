...
echo "=== OptiZomb v2 - Apply Patches ($TIER tier) ==="

if [ ! -d "$VANILLA_DIR/zombie" ]; then
    echo "ERROR: Vanilla source not found at $VANILLA_DIR"
    echo "Copy the new decompiled source to v2/vanilla/ first."
    exit 1
fi

# Clean src/ and copy vanilla as base
rm -rf "$SRC_DIR"
cp -rp "$VANILLA_DIR" "$SRC_DIR"
...
echo "Tier:  $TIER"
echo "Files: $TOTAL in v2/src/"
echo ""
echo "Next: bash v2/scripts/build-minimal.sh"
