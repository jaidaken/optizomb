...
INSTALLER_SRC="$ROOT_DIR/installer/src"
INSTALLER_BUILD="$BUILD_DIR/installer"
PATCHES_DIR="$BUILD_DIR/patches"
CONFIG_DIR="$ROOT_DIR/config"
OUTPUT_JAR="$BUILD_DIR/OptiZomb-v2-Lite-Installer.jar"

JBSDIFF_JAR="$ROOT_DIR/tools/jbsdiff-1.0.jar"
COMPRESS_JAR="$ROOT_DIR/tools/commons-compress-1.21.jar"
...
VERSION=$(cat "$PROJECT_DIR/version.txt" 2>/dev/null || echo "2.0.0")
VERSION=$(echo "$VERSION" | tr -d '[:space:]')

echo "=== OptiZomb v2 Installer Build (tier: $TIER, v$VERSION) ==="
...
if [ -d "$PZ_GAME_DIR/zombie" ]; then
    ...
    (cd "$VANILLA_BASE" && jar xf "$ROOT_DIR/libs/projectzomboid.jar")
