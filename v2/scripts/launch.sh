...
OUR_JAR="$BUILD_DIR/optizomb-v2-test.jar"

if [ ! -d "$BUILD_DIR/classes" ]; then
    echo "ERROR: Build not found. Run v2/scripts/build-minimal.sh first."
    exit 1
fi
...
cp "$ROOT_DIR/libs/projectzomboid.jar" "$OUR_JAR"
...
echo "=== OptiZomb v2 Launch ==="
echo "JAR:     $OUR_JAR"
echo "Runtime: $RUNTIME"
echo "Java:    $($JAVA -version 2>&1 | head -1)"
echo ""

CACHE_DIR="$ROOT_DIR/userdata"
mkdir -p "$CACHE_DIR"
