...
echo "=== OptiZomb v2 - Update Patches ($TIER tier) ==="

if [ ! -d "$VANILLA_DIR/zombie" ]; then
    echo "ERROR: Vanilla source not found. Copy new decompiled source to v2/vanilla/ first."
    exit 1
fi

if [ ! -d "$SRC_DIR/zombie" ]; then
    echo "ERROR: Source not found. Run v2/scripts/apply-patches.sh first, then make your edits."
    exit 1
fi
...
echo ""
echo "Remember to git add v2/patches/$TIER/ and commit."
